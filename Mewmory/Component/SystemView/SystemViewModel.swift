...
final class SystemViewModel: TickableViewModel {
    ...
    public override func tick(_ params: [AnyHashable : Any]?) {
        ...
        if !showHighCpuUsageWarning && highCpuUsage() {
            ...
            postEvent(name: "NotchViewModel.ShowTips", params: ["Tips": "⚠️ CPU占用过高: \(cpuUsage*100)%"])
            showHighCpuUsageWarning = true
        }
        
        if showHighCpuUsageWarning && normalCpuUsage() {
            Logger.log("now: \(Date.now)", category: .debug)
            postEvent(name: "NotchViewModel.ShowTips", params: ["Tips": "✅ CPU占用恢复正常: \(cpuUsage*100)%"])
            showNormalCpuUsageTips = true
        }
        
        if !showHighMemoryUsageWarning && highMemoryUsage() {
            Logger.log("now: \(Date.now)", category: .debug)
            postEvent(name: "NotchViewModel.ShowTips", params: ["Tips": "⚠️ 内存占用过高: \(memoryUsage*100)%"])
            showHighMemoryUsageWarning = true
        }
        
        if showHighMemoryUsageWarning && noamrlMemoryUsage() {
            Logger.log("now: \(Date.now)", category: .debug)
            postEvent(name: "NotchViewModel.ShowTips", params: ["Tips": "✅ 内存占用恢复正常: \(memoryUsage*100)%"])
            showNormalCpuUsageTips = true
        }
    }
